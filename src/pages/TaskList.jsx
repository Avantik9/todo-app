...
function TaskList(props) {
    ...
    const [filteredTasks , setFilteredTask] = useState(null)
    ...
    const  handleSearch = (e) =>{
          ...
          const filteredValue = allTasks.filter((task) => task.title.toLowerCase().includes(value.toLowerCase()));
          setFilteredTask(filteredValue)
    }
                    ...
                    </div>
                    {
                        filteredTasks?.map((task , id) => (
                            <div key={task.id} className='row bg-dark rounded-1 py-2 mb-1'>
                                <div className='col-lg-1'>{id+1}</div>
